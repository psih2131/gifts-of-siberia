    ...
    <div class="info-form-sec__form">
        ...
        <form action="">
            ...
            <div class="info-form-sec__form-inp">
                <input v-model="formCountry" type="text" :placeholder="$t('form.country')">
            </div>
            ...
            <div class="info-form-sec__form-inp info-form-sec__form-inp-textarea">
                <textarea v-model="formMessage" :placeholder="$t('form.requirementsAndWishes')"></textarea>
            </div>
...
const sendForm = async () => {
  try {
    const response = await $fetch('/api/send-form-export', {
      method: 'POST',
      body: {
        name: formName.value,
        country: formCountry.value || 'Пользыватель не указал',
        email: formEmail.value,
        phone: formPhone.value,
        politConfirm: formPolitCheckbox.value,
        spamConfirm: formSpamCheckbox.value,
        messageCurrent: formMessage.value || 'Пользыватель не указал',
