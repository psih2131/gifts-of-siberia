<template>
    <main class="main">
      ...
      <template v-if="pageData?.[0] && popupData?.[0]">
        ...
        <section class="info-hero-sec">
            <div class="container">
                ...
                <div class="breadcrumbs">
                    ...
                    <span class="breadcrumbs__text">{{ $t('breadcrumbs.legalEntities') }}</span>
                </div>
        ...
        <section class="info-advantages-sec" v-if="pageData[0].acf['sekcziya_-_chto_poluchaete']?.zagolovok">
            <div class="container">
                <h2 class="info-sec-title" v-html="pageData[0].acf['sekcziya_-_chto_poluchaete'].zagolovok"></h2>

                <div class="info-advantages-sec__wrapper">

                    <div class="info-advantages-sec__element" v-for="item in pageData[0].acf['sekcziya_-_chto_poluchaete'].spisok" :key="item">
                        <div class="info-advantages-sec__element-wrapper">
                            <div class="info-advantages-sec__element-header">
                                <img :src="item.ikonka.url" :alt="item.ikonka.url" class="info-advantages-sec__element-icon">
                                <p class="info-advantages-sec__element-title" v-html="item.zagolovok"></p>
                            </div>
                            <p class="info-advantages-sec__element-text" v-html="item.opisanie"></p>
                        </div>
                    </div>
        ...
        <section class="info-dirrections-sec" v-if="pageData[0].acf.sekcziya_napravleniya?.zagolovok">
            <div class="container">
        ...
        <section class="info-how-take-sec" v-if="pageData[0].acf['sekcziya_-_kak_poluchit_obrazczy']?.zagolovok">
            <div class="container">
                <h2 class="info-sec-title" v-html="pageData[0].acf['sekcziya_-_kak_poluchit_obrazczy'].zagolovok"></h2>
                <p class="info-how-take-sec__subtitle" v-html="pageData[0].acf['sekcziya_-_kak_poluchit_obrazczy'].podzagolovok"></p>
                <div class="info-how-take-sec__row">

                    <div class="info-how-take-sec__element" v-for="item in pageData[0].acf['sekcziya_-_kak_poluchit_obrazczy'].usloviya" :key="item">
                        <div class="info-how-take-sec__element-icon">
                            <img :src="item.ikonka.url" :alt="item.ikonka.alt">
                        </div>
                        <p class="info-how-take-sec__element-text" v-html="item.tekst"></p>
                    </div>
      ...
      <ContentNotTranslated v-else-if="!pageDataPending && !popupDataPending" />
...
const { data: popupData, pending: popupDataPending } = await useFetch(
  () => `${store.serverUrlDomainRequest}/wp-json/wp/v2/offer?slug=${route.params.id}${locale.value && locale.value !== 'ru' ? `&lang=${locale.value}` : ''}`,
...
function closeModal() {
  ...
  router.push(localePath('/yur-litsam'))
}
...
//HOOKS
onMounted(() => {
  const offer = popupData.value?.[0]
  if (offer) openPost(offer)
});

//SEO
const { t } = useI18n()
useHead(() => {
    const popup = popupData.value?.[0]
    if (!popup?.acf) return { title: t('nav.brand.giftsOfSiberia') }
    return {
        title: popup.acf.seo_title || popup.title?.rendered,
        meta: [
            { name: 'description', content: popup.acf.seo_description || t('common.defaultDescription') },
            { name: 'keywords', content: popup.acf.klyuchevaya_fraza || t('common.defaultKeywords') },
            { property: 'og:title', content: popup.acf.seo_title || popup.title?.rendered },
            { property: 'og:description', content: popup.acf.seo_description || t('common.defaultDescription') },
            { property: 'og:type', content: 'website' },
            { property: 'og:url', content: `${store.domainUrlCurrent}${route.fullPath}` },
            { property: 'og:image', content: popup.acf?.og_image?.url || 'http://syberia.gearsdpz.beget.tech/wp-content/uploads/2025/07/87baa9efe5d849e4f8da67fe01f9e029.jpg' },
            { name: 'twitter:card', content: 'summary_large_image' },
            { name: 'twitter:title', content: popup.acf.seo_title || popup.title?.rendered },
            { name: 'twitter:description', content: popup.acf.seo_description || t('common.defaultDescription') },
            { name: 'twitter:image', content: popup.acf?.og_image?.url || 'http://syberia.gearsdpz.beget.tech/wp-content/uploads/2025/07/87baa9efe5d849e4f8da67fe01f9e029.jpg' },
            { name: 'robots', content: popup.acf?.indeksacziya_v_poiskovyh_sistemah === 'index' ? 'index, follow' : 'noindex, nofollow' }
        ],
        link: [{ rel: 'canonical', href: `${store.domainUrlCurrent}${localePath(`/yur-litsam/${popup.acf?.canonical || route.params.id}`)}` }]
    }
})
</script>