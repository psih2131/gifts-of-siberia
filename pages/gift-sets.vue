        ...
        <section class="info-hero-sec gift-sets-hero-sec">
            <div class="container">
                ...
                <div class="breadcrumbs">
                    ...
                    <span class="breadcrumbs__text">{{ $t('breadcrumbs.giftSets') }}</span>
                </div>
                ...
                <div class="info-img-slider-sec__slider">
                    ...
                    <swiper-slide  v-for="item in pageData[0].acf.sekcziya_3_slajder_izobrazhenij" :key="item">
                        <div class="img-slider__element">
                            <img v-if="item?.izobrazhenie?.url" :src="item.izobrazhenie.url" :alt="item.izobrazhenie?.alt || ''" class="img-slider__img">
                        </div>
                    </swiper-slide>
...
const { data: pageData, pending } = await useFetch(
  () => `${store.serverUrlDomainRequest}/wp-json/wp/v2/pages?slug=gift-sets${locale.value && locale.value !== 'ru' ? `&lang=${locale.value}` : ''}`,
  { watch: [locale] }
)
